...
from config import MPI_SERIAL_DIR, MPI_SERIAL_PLACEHOLDER_DIR, MPI_SERIAL_HEURISTICS_DIR
...
def MPI_to_serial(mpi_db, mode='heuristics'):
    ...
    for program_name, paths in mpi_db.items():
        ...
        save_dir = os.path.join(MPI_SERIAL_HEURISTICS_DIR, program_name)
...
def lines_counting(mpi_db):
    db = {}
    for idx, (program_name, paths) in enumerate(mpi_db.items()):
        count = 0
        code, _, _ = load_file(paths['mpi_code'], load_by_line=True)
        for line in code:
            count += 1
        print(f'{idx} programs have been counted')
        db[program_name] = {'lines': count}
    write_to_json(db, 'mpi_lines_count.json')