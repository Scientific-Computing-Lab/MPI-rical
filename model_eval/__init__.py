...
def conf_matrix(reference, candidate):
...
# ref1 = 'int main ( int argc , char * * argv ) { mpi _init ( & argc , & argv ) ; int rank ; int com size ; mp i_ comm_ size ( mp i_ comm_ world , & com size ) ; mp i_ comm_ rank ( mp i_ comm_ world , & rank ) ; long long int local arraysize = ( ( ( long long int ) 1 ) < < 30 ) / com size ; long long int * local array = ( long long int * ) malloc ( local arraysize * ( sizeof ( long long int ) ) ) ; for ( long long int i = 0 ; i < local arraysize ; i ++ ) local array [ i ] = ( ( ( long long int ) rank ) * local arraysize ) + i ; long long int result ; uint64 _t starttime = clock _now ( ) ; mpi _p2 p_ reduce ( local array , & result , local arraysize , ( mp i_ datatype ) ( ( void * ) ( & o mp i_ mp i_ long_ long _int ) ) , mpi _sum , 0 , mp i_ comm_ world ) ; uint64 _t endtime = clock _now ( ) ; double time _in_ secs = ( ( double ) ( endtime - starttime ) ) / 512 000000 ; if ( rank == 0 ) printf ( ___str , result , time _in_ secs ) ; starttime = clock _now ( ) ; long long int local sum = 0 ; for ( long long int i = 0 ; i < local arraysize ; i ++ ) local sum += local array [ i ] ; mpi _reduce ( & local sum , & result , 1 , ( mp i_ datatype ) ( ( void * ) ('
# cand1 = 'int main ( int argc , char * * argv ) { int rank ; int com size ; mpi _init ( & argc , & argv ) ; mp i_ comm_ rank ( mp i_ comm_ world , & rank ) ; mp i_ comm_ size ( mp i_ comm_ world , & com size ) ; mp i_ comm_ size ( mp i_ comm_ world , & com size ) ; long long int local arraysize = ( ( ( long long int ) 1 ) < < 30 ) / com size ; long long int * local array = ( long long int * ) malloc ( local arraysize * ( sizeof ( long long int ) ) ) ; for ( long long int i = 0 ; i < local arraysize ; i ++ ) local array [ i ] = ( ( ( long long int ) rank ) * local arraysize ) + i ; long long int result ; uint64 _t starttime = clock _now ( ) ; uint64 _t endtime = clock _now ( ) ; double time _in_ secs = ( ( double ) ( endtime - starttime ) ) / 512 000000 ; if ( rank == 0 ) printf ( ___str , result , time _in_ secs ) ; starttime = clock _now ( ) ; long long int local sum = 0 ; for ( long long int i = 0 ; i < local arraysize ; i ++ ) local sum += local array [ i ] ; endtime = clock _now ( ) ; time _in_ secs = ( ( double ) ( endtime - starttime ) ) / 512 000000 ; if ( rank == 0 ) printf ( ___str , result , time _in_ secs ) ; free ( local array ) ; mp i_ finalize ( ) ; return 0 ; }'
